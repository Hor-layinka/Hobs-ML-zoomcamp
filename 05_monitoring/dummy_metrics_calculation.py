...
# create database and table for dummy variabbles
...
def prep_db():
	with psycopg.connect("host=localhost port=5432 user=postgres password=example", autocommit=True) as conn:
		...
		with psycopg.connect("host=localhost port=5432 user=postgres password=example") as conn:
			conn.execute(create_table_statement)

# calcilate dummy metrics values
def calculate_dummy_metrics_postgres(curr):
